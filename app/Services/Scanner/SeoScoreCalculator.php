...
class SeoScoreCalculator
{
    ...
    private function recommendations(array $data, array $scores): array
    {
        ...
        $this->addIf($items, ! data_get($data, 'technical_data.sitemap_xml.exists'), 'Technical SEO', 'sitemap.xml was not found.', 'Sitemaps help search and AI systems discover important URLs faster.', 'high', 'medium', 8, 'Publish an XML sitemap and reference it in robots.txt.', 'Generate /sitemap.xml from your CMS or framework and add "Sitemap: https://domain.com/sitemap.xml" to robots.txt.');
        $this->addIf($items, ! ($data['has_mobile_viewport'] ?? false), 'Technical SEO', 'Mobile viewport meta tag is missing.', 'Mobile rendering issues can reduce usability and search quality signals.', 'high', 'low', 8, 'Add a responsive viewport meta tag.', 'Add <meta name="viewport" content="width=device-width, initial-scale=1"> inside <head>.');
        ...
        $this->addIf($items, blank($data['canonical'] ?? null), 'On-Page SEO', 'Canonical URL is missing.', 'Canonicals reduce duplicate-content ambiguity.', 'medium', 'low', 4, 'Add a canonical tag.', 'Add <link rel="canonical" href="https://domain.com/page"> to the page head.');
        $this->addIf($items, str_contains(strtolower((string) ($data['robots_meta'] ?? '')), 'noindex'), 'Technical SEO', 'Robots meta contains noindex.', 'A noindex tag can prevent search and AI discovery entirely.', 'high', 'low', 15, 'Remove noindex if the page should rank.', 'Update the robots meta tag or CMS SEO settings to allow indexing.');
        ...
        $this->addIf($items, (float) data_get($data, 'content.content_html_ratio', 0) < 5, 'Content', 'Content-to-HTML ratio is low.', 'Heavy templates with little content weaken extraction quality.', 'low', 'medium', 3, 'Reduce template weight or add more useful text.', 'Trim unnecessary markup/scripts and strengthen body copy.');
        $this->addIf($items, ($data['internal_links_count'] ?? 0) === 0, 'Images & Links', 'No internal links were found.', 'Internal links help crawlers understand site structure and topic clusters.', 'medium', 'low', 5, 'Add relevant internal links.', 'Link to related services, articles, pricing, contact, or important conversion pages.');
        $this->addIf($items, ($data['images_missing_alt_count'] ?? 0) > 0, 'Images & Links', 'Some images are missing alt text.', 'Alt text improves accessibility and image/content interpretation.', 'medium', 'low', 4, 'Add descriptive alt attributes.', 'Use concise alt text for meaningful images; leave decorative images empty only when appropriate.');
        ...
        $this->addIf($items, $scores['social_score'] < 70, 'Social Preview', 'Social preview metadata is incomplete.', 'Social metadata improves previews and reinforces page entities.', 'low', 'low', 3, 'Add Open Graph and Twitter Card tags.', 'Include og:title, og:description, og:image, og:url, og:type, twitter:card, twitter:title, twitter:description, and twitter:image.');
        $this->addIf($items, ! data_get($data, 'performance_data.uses_compression'), 'Performance', 'Compression was not detected.', 'Compression reduces transfer size and improves load speed.', 'medium', 'medium', 5, 'Enable gzip or Brotli compression.', 'Enable compression in Apache/Nginx/CDN so text assets transfer faster.');
        $this->addIf($items, blank(data_get($data, 'performance_data.cache_control')), 'Performance', 'Cache-Control header is missing.', 'Caching reduces repeat load times for users and crawlers.', 'low', 'medium', 3, 'Add caching headers for static assets.', 'Configure long-lived cache headers for versioned CSS, JS, and images.');
