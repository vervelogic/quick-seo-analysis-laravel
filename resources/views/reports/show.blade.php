    ...
    <section class="bg-slate-50 py-10 sm:py-14">
        <div class="mx-auto grid max-w-7xl gap-6 px-5 sm:px-6 lg:grid-cols-[1fr_380px] lg:px-8">
            <div class="space-y-6">
                ...
                <section class="overflow-hidden rounded-xl border border-slate-200 bg-slate-950 p-6 shadow-sm sm:p-8"><div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><p class="text-sm font-semibold uppercase tracking-[0.18em] text-teal-300">Next Step</p><h2 class="mt-2 text-2xl font-black tracking-tight text-white">Want us to fix these issues?</h2><p class="mt-3 max-w-2xl text-sm leading-6 text-slate-300">Turn this report into a prioritized implementation plan for technical SEO, content, AI visibility, GEO and AEO.</p></div><div class="flex flex-col gap-3 sm:flex-row lg:flex-col xl:flex-row"><a href="mailto:hello@vervelogic.com?subject=QSA%20Strategy%20Call%20for%20{{ urlencode($host) }}" class="inline-flex min-h-12 items-center justify-center rounded-lg bg-white px-5 font-bold text-slate-950 hover:bg-slate-100">Book Strategy Call</a><a href="https://wa.me/?text={{ urlencode('I want help fixing the QSA report issues for '.$effectiveUrl) }}" target="_blank" rel="noopener" class="inline-flex min-h-12 items-center justify-center rounded-lg bg-teal-500 px-5 font-bold text-white hover:bg-teal-400">WhatsApp</a></div></div></section>
            ...
            <aside class="space-y-6 lg:sticky lg:top-6 lg:h-fit">
                ...
                <section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><h2 class="text-xl font-black tracking-tight text-slate-950">Lead Capture CTA</h2><p class="mt-2 text-sm leading-6 text-slate-600">Save contact details for follow-up and future email report delivery.</p><form method="POST" action="{{ route('lead.capture') }}" class="mt-5 space-y-4">@csrf<input type="hidden" name="scan_uuid" value="{{ $scan->uuid }}"><div><label class="text-sm font-semibold text-slate-800" for="name">Name</label><input id="name" name="name" value="{{ old('name') }}" class="mt-1 w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-600 focus:ring-blue-600"></div><div><label class="text-sm font-semibold text-slate-800" for="email">Email</label><input id="email" name="email" type="email" value="{{ old('email') }}" class="mt-1 w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-600 focus:ring-blue-600" required>@error('email')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div><div><label class="text-sm font-semibold text-slate-800" for="phone">Phone</label><input id="phone" name="phone" value="{{ old('phone') }}" class="mt-1 w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-600 focus:ring-blue-600"></div><div><label class="text-sm font-semibold text-slate-800" for="company_name">Company</label><input id="company_name" name="company_name" value="{{ old('company_name') }}" class="mt-1 w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-600 focus:ring-blue-600"></div><button class="w-full rounded-lg bg-slate-950 px-5 py-3 font-bold text-white hover:bg-slate-800" type="submit">Save details</button></form></section>
            </aside>
        </div>
    </section>
</x-layouts.app>
