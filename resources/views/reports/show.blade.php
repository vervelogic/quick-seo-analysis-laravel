<x-layouts.app :title="'Visibility Report - '.$scan->normalized_url">
    @php
        ...
        $usesHttps = ($result?->uses_https) ?? strtolower($scheme) === 'https';
        $scanFailed = $scan->status === 'failed' || (($result?->is_reachable) === false) || ! $result;
        $fetchError = data_get($raw, 'error') ?: $scan->error_message;
        ...
        $scores = [
            'Overall Visibility' => $scanFailed ? 0 : ($scoreBreakdown['overall_visibility_score'] ?? $result?->score ?? 0),
            'SEO' => $scanFailed ? 0 : ($scoreBreakdown['seo_score'] ?? $scoreBreakdown['overall_score'] ?? $result?->score ?? 0),
            'AI Visibility' => $scanFailed ? 0 : ($scoreBreakdown['ai_visibility_score'] ?? data_get($aiVisibility, 'score', 0)),
            'GEO' => $scanFailed ? 0 : ($scoreBreakdown['geo_score'] ?? data_get($geo, 'score', 0)),
            'AEO' => $scanFailed ? 0 : ($scoreBreakdown['aeo_score'] ?? data_get($aeo, 'score', 0)),
        ];
        ...
        <div class="relative mx-auto grid max-w-7xl gap-8 px-5 sm:px-6 lg:grid-cols-[1fr_380px] lg:px-8">
            <div class="flex flex-col justify-center">
                ...
                <div class="mt-6 flex flex-wrap gap-3">
                    ...
                    <span class="rounded-full px-4 py-2 text-sm font-bold ring-1 {{ $usesHttps ? 'bg-teal-400/15 text-teal-100 ring-teal-300/30' : 'bg-amber-400/15 text-amber-100 ring-amber-300/30' }}">HTTPS {{ $usesHttps ? 'enabled' : 'not detected' }}</span>
                </div>
            </div>
            ...
            <div class="rounded-xl bg-white p-6 shadow-2xl shadow-slate-950/30">
                ...
                <p class="mt-4 text-sm leading-6 text-slate-600">{{ $scanFailed ? 'No score was calculated because the page could not be fetched for analysis.' : 'Combined SEO, AI Visibility, GEO and AEO readiness based on the current scan.' }}</p>
            </div>
        </div>
    </section>
    ...
    <section class="bg-slate-50 py-10 sm:py-14">
        <div class="mx-auto grid max-w-7xl gap-6 px-5 sm:px-6 lg:grid-cols-[1fr_380px] lg:px-8">
            <div class="space-y-6">
                ...
                @if ($scanFailed)
                    <section class="rounded-xl border border-red-200 bg-red-50 p-5 shadow-sm sm:p-6">
                        <p class="text-sm font-semibold uppercase tracking-[0.18em] text-red-700">Scan could not be completed</p>
                        <h2 class="mt-2 text-2xl font-black tracking-tight text-red-950">QSA could not fetch readable page content for this URL.</h2>
                        <p class="mt-3 max-w-3xl text-sm leading-6 text-red-900">This can happen when the website blocks server-side requests, times out, requires browser verification, blocks the scanner user agent or IP, redirects in a protected way, or is unavailable from the VPS network.</p>
                        <div class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                            <div class="rounded-lg bg-white p-4 ring-1 ring-red-100"><p class="text-xs font-bold uppercase tracking-[0.14em] text-red-700">Requested URL</p><p class="mt-2 break-words text-sm font-black text-slate-950">{{ $requestedUrl }}</p></div>
                            <div class="rounded-lg bg-white p-4 ring-1 ring-red-100"><p class="text-xs font-bold uppercase tracking-[0.14em] text-red-700">Final URL</p><p class="mt-2 break-words text-sm font-black text-slate-950">{{ $effectiveUrl }}</p></div>
                            <div class="rounded-lg bg-white p-4 ring-1 ring-red-100"><p class="text-xs font-bold uppercase tracking-[0.14em] text-red-700">HTTP</p><p class="mt-2 text-sm font-black text-slate-950">{{ $result?->http_status ?? 'N/A' }}</p></div>
                            <div class="rounded-lg bg-white p-4 ring-1 ring-red-100"><p class="text-xs font-bold uppercase tracking-[0.14em] text-red-700">HTTPS</p><p class="mt-2 text-sm font-black text-slate-950">{{ $usesHttps ? 'Enabled' : 'Not detected' }}</p></div>
                        </div>
                        @if ($fetchError)
                            <div class="mt-4 rounded-lg bg-white p-4 text-sm leading-6 text-red-900 ring-1 ring-red-100"><span class="font-black">Scanner message:</span> {{ $fetchError }}</div>
                        @endif
                    </section>
                @endif

                <section class="{{ $sectionCard }}">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p class="text-sm font-semibold uppercase tracking-[0.18em] text-blue-700">Executive Summary</p><h2 class="mt-1 text-2xl font-black tracking-tight text-slate-950">{{ $scanFailed ? 'Fetch Summary' : 'Visibility Dashboard' }}</h2></div><span class="w-fit rounded-full px-4 py-2 text-sm font-bold ring-1 {{ $softPill($overall) }}">Overall {{ $overall }}/100</span></div>
                    <div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                        @foreach ([
                            ...
                            'Opportunities' => $scanFailed ? 'N/A' : $opportunities->count(),
