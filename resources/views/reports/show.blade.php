        ...
        <div class="relative mx-auto grid max-w-7xl gap-8 px-5 sm:px-6 lg:grid-cols-[1fr_380px] lg:px-8">
            ...
            <div class="rounded-xl bg-white p-6 shadow-2xl shadow-slate-950/30">
                ...
                <div class="mt-5 h-3 rounded-full bg-slate-100">
                    <div class="h-3 rounded-full {{ $scoreBg($overall) }}" style="width: {{ max(0, min(100, $overall)) }}%"></div>
                </div>
            ...
            <div class="space-y-6">
                ...
                <section class="{{ $sectionCard }}">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            <p class="text-sm font-semibold uppercase tracking-[0.18em] text-blue-700">Executive Summary</p>
                            <h2 class="mt-1 text-2xl font-black tracking-tight text-slate-950">Visibility Dashboard</h2>
                        </div>
                        <span class="w-fit rounded-full px-4 py-2 text-sm font-bold ring-1 {{ $softPill($overall) }}">Overall {{ $overall }}/100</span>
                    </div>
                    <div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                        @foreach ([
                            ...
                            <div class="{{ $metricCard }}">
                                ...
                                <p class="mt-2 break-words text-2xl font-black text-slate-950">{{ $value }}</p>
                            </div>
                        @endforeach
                    </div>
                </section>
                ...
                <section class="{{ $sectionCard }}">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                            <p class="text-sm font-semibold uppercase tracking-[0.18em] text-teal-700">Score Breakdown</p>
                            <h2 class="mt-1 text-2xl font-black tracking-tight text-slate-950">Overall Visibility, SEO, AI Visibility, GEO and AEO</h2>
                        </div>
                    </div>
                    <div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
                        @foreach ($scores as $scoreLabel => $score)
                            <div class="rounded-lg border border-slate-200 bg-gradient-to-b from-white to-slate-50 p-4 shadow-sm">
                                <p class="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">{{ $scoreLabel }}</p>
                                <p class="mt-3 text-3xl font-black {{ $scoreText($score) }}">{{ (int) $score }}</p>
                                <div class="mt-3 h-2 rounded-full bg-slate-100"><div class="h-2 rounded-full {{ $scoreBg($score) }}" style="width: {{ max(0, min(100, (int) $score)) }}%"></div></div>
                            </div>
                        @endforeach
                    </div>
                    <div class="mt-6 grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
                        ...
                        <div class="rounded-lg border border-slate-200 bg-white p-5">
                            ...
                            <div class="mt-5 space-y-4">
                                @foreach ($scores as $scoreLabel => $score)
                                    <div>
                                        <div class="flex justify-between gap-4 text-sm"><span class="font-bold text-slate-800">{{ $scoreLabel }}</span><span class="font-black">{{ (int) $score }}</span></div>
                                        <div class="mt-2 h-3 rounded-full bg-slate-100"><div class="h-3 rounded-full {{ $scoreBg($score) }}" style="width: {{ max(0, min(100, (int) $score)) }}%"></div></div>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                ...
                <section class="{{ $sectionCard }}">
                    ...
                    <p class="mt-2 text-sm leading-6 text-slate-600">Signals that help AI systems understand the brand, entity, expertise and trust context.</p>
                    <div class="mt-5 grid gap-3 sm:grid-cols-2">
                        @forelse ((array) data_get($aiVisibility, 'signals', []) as $signal => $ok)
                            <div class="flex items-center justify-between gap-4 rounded-lg border border-slate-200 bg-white p-4">
                                ...
                                <span class="rounded-full px-3 py-1 text-sm font-bold ring-1 {{ $statusPill((bool) $ok) }}">{{ $ok ? 'Detected' : 'Missing' }}</span>
                            </div>
                        @empty
                            <div class="rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-600">No AI visibility signals were recorded for this scan.</div>
                        @endforelse
                    </div>
                </section>

                <section class="{{ $sectionCard }}">
                    <h2 class="text-2xl font-black tracking-tight text-slate-950">GEO</h2>
                    <p class="mt-2 text-sm leading-6 text-slate-600">Generative Engine Optimization signals for conversational discovery and semantic coverage.</p>
                    <div class="mt-5 grid gap-3 sm:grid-cols-2">
                        @forelse ((array) data_get($geo, 'signals', []) as $signal => $ok)
                            <div class="flex items-center justify-between gap-4 rounded-lg border border-slate-200 bg-white p-4">
                                <span class="font-semibold text-slate-900">{{ $label($signal) }}</span>
                                <span class="rounded-full px-3 py-1 text-sm font-bold ring-1 {{ $statusPill((bool) $ok) }}">{{ $ok ? 'Ready' : 'Improve' }}</span>
                            </div>
                        @empty
                            <div class="rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-600">No GEO signals were recorded for this scan.</div>
                        @endforelse
                    </div>
                </section>

                <section class="{{ $sectionCard }}">
                    <h2 class="text-2xl font-black tracking-tight text-slate-950">AEO</h2>
                    <p class="mt-2 text-sm leading-6 text-slate-600">Answer Engine Optimization checks for direct answers, FAQ coverage and snippet readiness.</p>
                    <div class="mt-5 grid gap-3 sm:grid-cols-2">
                        @forelse ((array) data_get($aeo, 'signals', []) as $signal => $ok)
                            <div class="flex items-center justify-between gap-4 rounded-lg border border-slate-200 bg-white p-4">
                                <span class="font-semibold text-slate-900">{{ $label($signal) }}</span>
                                <span class="rounded-full px-3 py-1 text-sm font-bold ring-1 {{ $statusPill((bool) $ok) }}">{{ $ok ? 'Ready' : 'Missing' }}</span>
                            </div>
                        @empty
                            <div class="rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-600">No AEO signals were recorded for this scan.</div>
                        @endforelse
                    </div>
                </section>
                ...
                <section class="{{ $sectionCard }}">
                    ...
                    <div class="mt-5 grid gap-3 sm:grid-cols-2">
                        @foreach ([
                            ...
                            <div class="flex items-center justify-between gap-4 rounded-lg border border-slate-200 bg-white p-4"><span class="font-semibold text-slate-900">{{ $technicalLabel }}</span><span class="rounded-full px-3 py-1 text-sm font-bold ring-1 {{ $statusPill((bool) $ok) }}">{{ $ok ? 'Passed' : 'Needs work' }}</span></div>
                        @endforeach
                    </div>
                </section>
                ...
                <section class="{{ $sectionCard }}">
                    ...
                    <div class="mt-5 grid gap-4 sm:grid-cols-3">
                        @foreach ([
                            ...
                            <div class="{{ $metricCard }}"><p class="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">{{ $contentLabel }}</p><p class="mt-2 text-2xl font-black text-slate-950">{{ $value }}</p></div>
                        @endforeach
                    </div>
                </section>
                ...
                <section class="{{ $sectionCard }}">
                    ...
                    <div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                        @foreach ([
                            ...
                            <div class="{{ $metricCard }}"><p class="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">{{ $metric }}</p><p class="mt-2 text-2xl font-black text-slate-950">{{ $value }}</p></div>
                        @endforeach
                    </div>
                </section>
                ...
                <section class="{{ $sectionCard }}">
                    ...
                    <div class="mt-5 rounded-lg border border-slate-200 bg-white p-5">
                        ...
                        <p class="mt-2 text-sm text-slate-600">Schema types: <span class="font-bold text-slate-950">{{ implode(', ', data_get($structured, 'types', [])) ?: 'None detected' }}</span></p>
                        <p class="mt-2 text-sm text-slate-600">Microdata/RDFa: <span class="font-bold text-slate-950">{{ data_get($structured, 'has_microdata') || data_get($structured, 'has_rdfa') ? 'Detected' : 'Not detected' }}</span></p>
                    </div>
                </section>
                ...
                <section class="{{ $sectionCard }}">
                    ...
                    <div class="mt-5 grid gap-4 lg:grid-cols-2">
                        <div class="rounded-lg border border-slate-200 bg-white p-5"><h3 class="font-bold text-slate-950">Open Graph</h3><p class="mt-3 text-sm text-slate-600">{{ collect(data_get($social, 'open_graph', []))->filter()->count() }} of 5 tags detected.</p></div>
                        <div class="rounded-lg border border-slate-200 bg-white p-5"><h3 class="font-bold text-slate-950">Twitter Card</h3><p class="mt-3 text-sm text-slate-600">{{ collect(data_get($social, 'twitter_card', []))->filter()->count() }} of 4 tags detected.</p></div>
                    </div>
                </section>
                ...
                <section class="{{ $sectionCard }}">
                    ...
                    <div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                        @foreach ([
                            ...
                            <div class="{{ $metricCard }}"><p class="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">{{ $metric }}</p><p class="mt-2 break-words text-lg font-black text-slate-950">{{ $value }}</p></div>
                        @endforeach
                    </div>
                </section>
                ...
                <section class="{{ $sectionCard }}">
                    ...
                    <div class="mt-5 grid gap-3 sm:grid-cols-2">
                        @foreach (['strict_transport_security', 'x_frame_options', 'x_content_type_options', 'content_security_policy', 'referrer_policy'] as $header)
                            <div class="flex items-center justify-between gap-4 rounded-lg border border-slate-200 bg-white p-4"><span class="font-semibold text-slate-900">{{ $label($header) }}</span><span class="rounded-full px-3 py-1 text-sm font-bold ring-1 {{ $statusPill(filled(data_get($security, $header))) }}">{{ filled(data_get($security, $header)) ? 'Present' : 'Missing' }}</span></div>
                        @endforeach
                    </div>
                </section>
                ...
                <section class="{{ $sectionCard }}">
                    ...
                    <div class="mt-5 overflow-hidden rounded-lg border border-slate-200 bg-white">
                        <table class="min-w-full divide-y divide-slate-200 text-sm">
                            <thead class="bg-slate-50 text-left"><tr><th class="px-4 py-3 font-bold text-slate-700">Date</th><th class="px-4 py-3 font-bold text-slate-700">Status</th><th class="px-4 py-3 font-bold text-slate-700">Score</th></tr></thead>
                            <tbody class="divide-y divide-slate-100">
                                @forelse ($history ?? collect() as $item)
                                    <tr><td class="px-4 py-3 text-slate-700">{{ $item->created_at?->format('M j, Y g:i A') }}</td><td class="px-4 py-3 font-semibold capitalize text-slate-900">{{ $item->status }}</td><td class="px-4 py-3 font-black text-slate-950">{{ $item->result?->score ?? 'N/A' }}</td></tr>
                                @empty
                                    <tr><td colspan="3" class="px-4 py-5 text-center text-slate-500">No previous scans for this domain yet.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
            ...
            <aside class="space-y-6 lg:sticky lg:top-6 lg:h-fit">
                <section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <p class="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Website Preview</p>
                            <h2 class="mt-1 break-words text-xl font-black tracking-tight text-slate-950">{{ $host }}</h2>
                        </div>
                        <span class="rounded-full px-3 py-1 text-xs font-bold ring-1 {{ $result?->uses_https ? 'bg-teal-50 text-teal-800 ring-teal-100' : 'bg-amber-50 text-amber-800 ring-amber-100' }}">{{ strtoupper($scheme) }}</span>
                    </div>
