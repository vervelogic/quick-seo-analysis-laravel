@php
    ...
    $metadata = (array) data_get($reportData, 'metadata', []);
    $branding = (array) data_get($reportData, 'branding', []);
    $scores = (array) data_get($reportData, 'scores', []);
    $scoreExplanations = (array) data_get($reportData, 'score_explanations', []);
    $executive = (array) data_get($reportData, 'executive_summary', []);
    $opportunities = (array) data_get($reportData, 'opportunity_scores', []);
    $searchFocus = (array) data_get($reportData, 'current_search_focus', []);
    $keywordFocus = (array) data_get($reportData, 'keyword_focus', []);
    $aiVisibility = (array) data_get($reportData, 'ai_visibility', []);
    $geo = (array) data_get($reportData, 'geo', []);
    $aeo = (array) data_get($reportData, 'aeo', []);
    $citation = (array) data_get($reportData, 'citation_readiness', []);
    $topic = (array) data_get($reportData, 'topic_intelligence', []);
    $coverage = (array) data_get($reportData, 'content_coverage', []);
    $competitor = (array) data_get($reportData, 'competitor_summary', []);
    $technical = (array) data_get($reportData, 'technical_seo', []);
    $recommendations = (array) data_get($reportData, 'recommendations', []);
    $priorityMatrix = collect(data_get($recommendations, 'priority_matrix', []))->filter(fn ($item) => is_array($item) || is_object($item));
    $topActions = collect(data_get($recommendations, 'top_priority_actions', []))->filter(fn ($item) => is_array($item) || is_object($item));
    ...
    $companyWebsite = data_get($branding, 'website') ?: ($company->website_url ?: $company->domain ?: 'Not available');
    ...
    $finalUrl = data_get($metadata, 'final_url') ?: ($scan->normalized_url ?: $scan->url ?: 'Not available');
    ...
    $safeText = function ($value, string $default = 'Not available'): string {
        if ($value instanceof \Carbon\CarbonInterface) {
            return $value->timezone(config('app.timezone'))->format('d M Y, h:i A').' IST';
        }
        ...
        if (is_array($value) || $value instanceof \Illuminate\Support\Collection) {
            $flat = collect($value)->flatten()->filter(fn ($item) => is_scalar($item) && trim((string) $item) !== '')->map(fn ($item) => trim((string) $item))->take(8)->values();
            return $flat->isNotEmpty() ? $flat->implode(', ') : $default;
        }

        return $default;
    };

    $score = fn (string $key, int $default = 0): int => max(0, min(100, (int) data_get($scores, $key, $default)));
    $explanation = fn (string $key, string $field, string $default = 'Review this signal.'): string => $safeText(data_get($scoreExplanations, $key.'.'.$field), $default);
    $asList = fn ($value): string => $safeText($value, 'Not detected');
...
<head>
    ...
    <title>Visibility Report - {{ $safeText($domain, 'Website') }}</title>
    <style>
    ...
    <main class="sheet">
        <section class="page cover">
            ...
            <div class="content">
                <div class="brand-row">
                    <div class="brand">
                        @if ($logoPath)
                            <img class="logo" src="{{ asset('storage/'.$logoPath) }}" alt="{{ $safeText($companyName, 'Company') }}">
                        @else
                            <span class="logo-fallback">{{ \Illuminate\Support\Str::of($safeText($companyName, 'C'))->substr(0, 1)->upper() }}</span>
                        @endif
                        <div>
                            <p class="eyebrow">Prepared by</p>
                            <p style="font-size:22px;font-weight:900;">{{ $safeText($companyName, 'Company') }}</p>
                            <p class="muted">{{ $safeText($companyWebsite) }}</p>
                        </div>
                    </div>
                ...
                <div class="meta-grid">
                    <div class="meta-card"><b>Scanned URL</b>{{ $safeText($finalUrl) }}</div>
                    <div class="meta-card"><b>Domain</b>{{ $safeText($domain, 'Website') }}</div>
                    <div class="meta-card"><b>Generated</b>{{ $generatedAt->format('d M Y, h:i A') }} IST</div>
                    <div class="meta-card"><b>Audit Type</b>{{ \Illuminate\Support\Str::of($safeText($scanMode, 'current visibility'))->replace('_', ' ')->title() }}</div>
                </div>
            ...
            <div class="footer"><span>{{ $safeText($footerText) }}</span><span>{{ $safeText($company->contact_email, '') }}</span></div>
        </section>
        ...
        <section class="page">
            ...
            <div class="content">
                ...
                <div class="grid-2 section">
                    <div class="card"><h3>Search Visibility Potential</h3><p>{{ $safeText(data_get($executive, 'search_visibility_potential', data_get($executive, 'current_visibility_status')), 'Needs review') }}</p></div>
                    <div class="card"><h3>Commercial Intent Strength</h3><p>{{ $safeText(data_get($executive, 'commercial_intent_strength', data_get($opportunities, 'commercial_intent')), 'Not enough data') }}</p></div>
                    <div class="card"><h3>AI Visibility</h3><p>{{ $safeText(data_get($executive, 'ai_visibility', data_get($executive, 'ai_visibility_gap')), 'Review AI readiness signals') }}</p></div>
                    <div class="card"><h3>Biggest Opportunity</h3><p>{{ $safeText(data_get($executive, 'biggest_opportunity', data_get($opportunities, 'biggest_opportunity')), 'Improve priority visibility signals.') }}</p></div>
                </div>
                ...
                <div class="section">
                    ...
                    @forelse ($topActions->take(6) as $action)
                        <div class="recommendation">
                            <h3>{{ $safeText(data_get($action, 'issue', data_get($action, 'title')), 'Improvement opportunity') }}</h3>
                            <p>{{ $safeText(data_get($action, 'recommendation', data_get($action, 'how_to_fix')), 'Review and improve this item.') }}</p>
                        </div>
                    @empty
            ...
            <div class="footer"><span>{{ $safeText($footerText) }}</span><span>{{ $generatedAt->format('d M Y') }}</span></div>
        </section>
            ...
            <div class="content">
                ...
                @foreach ($scoreRows as $label => $row)
                    <div class="card section">
                        ...
                        <div class="bar"><span style="width: {{ max(0, min(100, $row['score'])) }}%;"></span></div>
                        <p>{{ $safeText($row['summary'], 'Review this score.') }}</p>
        ...
        <section class="page">
            ...
            <div class="content">
                ...
                <div class="grid-2">
                    <div class="card"><h3>Current Search Focus</h3><p>{{ $safeText(data_get($searchFocus, 'primary_focus', data_get($searchFocus, 'primary_search_focus')), 'Not detected') }}</p><p class="muted" style="margin-top:8px;">Intent: {{ $safeText(data_get($searchFocus, 'search_intent'), 'Not detected') }}</p></div>
                    <div class="card"><h3>AI Visibility Summary</h3><p>{{ $safeText(data_get($aiVisibility, 'summary', data_get($aiVisibility, 'likely_ai_citation_chance')), 'Not enough data') }}</p></div>
                    <div class="card"><h3>GEO Readiness</h3><p>{{ $safeText(data_get($geo, 'summary'), 'Review generative engine readiness signals.') }}</p></div>
                    <div class="card"><h3>AEO Readiness</h3><p>{{ $safeText(data_get($aeo, 'summary'), 'Review answer engine optimization signals.') }}</p></div>
                    <div class="card"><h3>AI Citation Readiness</h3><p>{{ $safeText(data_get($citation, 'summary', data_get($citation, 'likely_ai_citation_chance')), 'Review trust and citation signals.') }}</p></div>
                    <div class="card"><h3>Competitor Summary</h3><p>{{ $safeText(data_get($competitor, 'summary'), 'Future-ready competitor gap analysis placeholder. No competitor data is connected yet.') }}</p></div>
                </div>

                <div class="grid-2 section">
                    <div class="card"><h3>Topic Intelligence</h3><p><strong>Primary:</strong> {{ $asList(data_get($topic, 'primary_topics', [])) }}</p><p class="muted" style="margin-top:8px;"><strong>Entities:</strong> {{ $asList(data_get($topic, 'entities', [])) }}</p></div>
                    <div class="card"><h3>Content Coverage</h3><p>{{ $safeText(data_get($coverage, 'summary'), 'Review topic coverage and missing content opportunities.') }}</p><p class="muted" style="margin-top:8px;"><strong>Coverage:</strong> {{ $safeText(data_get($coverage, 'coverage_score', data_get($coverage, 'score')), 'N/A') }}</p></div>
                </div>
            </div>
            <div class="footer"><span>{{ $safeText($footerText) }}</span><span>{{ $generatedAt->format('d M Y') }}</span></div>
        </section>
            ...
            <section class="page">
                ...
                <div class="content">
                    ...
                    <div class="grid-3 section">
                        <div class="card"><h3>Overall Alignment</h3><p class="accent" style="font-size:24px;font-weight:900;">{{ (int) data_get($keywordFocus, 'overall_score', 0) }}/100</p></div>
                        <div class="card"><h3>Total Keywords</h3><p>{{ (int) data_get($keywordFocus, 'summary.total', collect(data_get($keywordFocus, 'keywords', []))->count()) }}</p></div>
                        <div class="card"><h3>Needs Support</h3><p>{{ $safeText(data_get($keywordFocus, 'summary.weak_or_missing'), 'N/A') }}</p></div>
                    </div>
                    <table>
                        <thead><tr><th>Keyword</th><th>Score</th><th>Status</th><th>Found In</th><th>Suggested Fix</th></tr></thead>
                        <tbody>
                            @forelse (collect(data_get($keywordFocus, 'keywords', []))->filter(fn ($row) => is_array($row) || is_object($row)) as $row)
                                <tr>
                                    <td>{{ $safeText(data_get($row, 'keyword')) }}</td>
                                    <td>{{ (int) data_get($row, 'alignment_score', data_get($row, 'score', 0)) }}</td>
                                    <td>{{ $safeText(data_get($row, 'status'), 'N/A') }}</td>
                                    <td>{{ $asList(data_get($row, 'found_in', [])) }}</td>
                                    <td>{{ $safeText(data_get($row, 'suggested_on_page_fix', data_get($row, 'suggested_fix')), 'Add stronger page support for this keyword or a close variation.') }}</td>
                                </tr>
                            @empty
            ...
            <div class="content">
                ...
                <table>
                    ...
                    <tbody>
                        @forelse ($priorityMatrix->take(12) as $item)
                            <tr>
                                <td>{{ $safeText(data_get($item, 'issue'), 'Improvement opportunity') }}</td>
                                <td>{{ $safeText(data_get($item, 'category'), 'General') }}</td>
                                <td>{{ $safeText(data_get($item, 'impact'), 'Medium') }}</td>
                                <td>{{ $safeText(data_get($item, 'difficulty'), 'Medium') }}</td>
                                <td>{{ $safeText(data_get($item, 'estimated_gain'), 'Not estimated') }}</td>
                                <td>{{ $safeText(data_get($item, 'how_to_fix', data_get($item, 'recommendation')), 'Review and improve this item.') }}</td>
                            </tr>
                        @empty
                ...
                <div class="section">
                    ...
                    @forelse ($groupedRecommendations as $category => $items)
                        <div class="recommendation">
                            <h3>{{ $safeText($category, 'General') }}</h3>
                            <ul class="list">
                                @foreach (collect($items)->filter(fn ($item) => is_array($item) || is_object($item))->take(3) as $item)
                                    <li>{{ $safeText(data_get($item, 'issue', data_get($item, 'recommendation')), 'Improvement opportunity') }}</li>
                                @endforeach
                            </ul>
                        </div>
                    @empty
            ...
            <div class="content">
                ...
                <div class="grid-2">
                    @forelse ($roadmap as $week => $items)
                        <div class="card">
                            <h3>{{ \Illuminate\Support\Str::of($safeText($week, 'Roadmap'))->replace('_', ' ')->title() }}</h3>
                            <ul class="list">
                                @foreach (collect($items)->take(6) as $item)
                                    <li>{{ $safeText(is_array($item) || is_object($item) ? data_get($item, 'action', data_get($item, 'issue')) : $item, 'Roadmap action') }}</li>
                                @endforeach
                            </ul>
                        </div>
                    @empty
                    ...
                    <table>
                        <tbody>
                            @forelse ($technical as $label => $value)
                                ...
                                <tr><th style="width:34%;">{{ \Illuminate\Support\Str::of($safeText($label, 'Signal'))->replace('_', ' ')->title() }}</th><td>{{ $safeText($value) }}</td></tr>
                            @empty
                                <tr><th>HTTP Status</th><td>{{ $safeText($result?->http_status, 'N/A') }}</td></tr>
                                <tr><th>HTTPS</th><td>{{ $result?->uses_https ? 'Enabled' : 'Not detected' }}</td></tr>
                                <tr><th>Title</th><td>{{ $safeText($result?->title, 'Not detected') }}</td></tr>
                                <tr><th>Meta Description</th><td>{{ $safeText($result?->meta_description, 'Not detected') }}</td></tr>
                                <tr><th>H1 Count</th><td>{{ $safeText($result?->h1_count, 'N/A') }}</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="footer"><span>{{ $whiteLabelActive ? $safeText($companyName, 'Company') : $safeText($companyName, 'Company').' | Generated by QSA' }}</span><span>{{ $generatedAt->format('d M Y, h:i A') }} IST</span></div>
        </section>
    </main>
</body>
</html>
